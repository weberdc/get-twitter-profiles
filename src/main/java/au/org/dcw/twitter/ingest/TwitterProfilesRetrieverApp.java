...
public final class TwitterProfilesRetrieverApp {
    ...
    private static final int TWITTER_PROFILE_FETCH_BATCH_SIZE = 100;
    static {
    ...
    private String screenNamesFile = null;
    private List<String> screenNames = null;
    private String outputDir = "./profiles";
    private String credentialsFile = "./twitter.properties";
    private boolean debug = false;


    public TwitterProfilesRetrieverApp(String screenNamesFile, String outputDir, String credentialsFile, boolean debug) {
        this.screenNamesFile = screenNamesFile;
    ...
    public static void main(String[] args) throws IOException {
        ...
        new TwitterProfilesRetrieverApp(screenNamesFile, outputDir, credentialsFile, debug).run();
    }
    ...
    public void run() throws IOException {
        ...
        final Configuration config = TwitterProfilesRetrieverApp.buildTwitterConfiguration(credentialsFile, debug);
        Twitter twitter = new TwitterFactory(config).getInstance();
        twitter.addRateLimitStatusListener(rateLimitStatusListener);
        ObjectMapper json = new ObjectMapper();

        for (List<String> batch : Lists.partition(screenNames, TWITTER_PROFILE_FETCH_BATCH_SIZE)) {

            try {
                LOG.info("Looking up {} users' profiles", batch.size());

                // ask Twitter for profiles
                ResponseList<User> profiles = twitter.lookupUsers(batch.toArray(new String[batch.size()]));

                // extract the raw JSON
                String rawJsonProfiles = TwitterObjectFactory.getRawJSON(profiles);

                // traverse the raw JSON (rather than the Twitter4j structures)
                JsonNode profilesJsonNode = json.readTree(rawJsonProfiles);
                profilesJsonNode.forEach (profileNode ->  {

                    // extract vars for logging and filename creation
                    String profileId = profileNode.get("id_str").asText();
                    String screenName = profileNode.get("screen_name").asText();

                    String fileName = outputDir + "/profile-" + profileId + ".json";
    ...
    private List<String> loadScreenNames() throws IOException {
     ...
     */
    private static Configuration buildTwitterConfiguration(String credentialsFile, boolean debug) throws IOException {
    ...
    private static Properties loadProxyProperties() {
        ...
        if (new File(proxyFile).exists()) {
            ...
            if (success && !properties.containsKey("http.proxyPassword")) {
                final char[] password = System.console().readPassword("Please type in your proxy password: ");
...
//         final TwitterProfilesRetrieverApp theApp = new TwitterProfilesRetrieverApp();
//
